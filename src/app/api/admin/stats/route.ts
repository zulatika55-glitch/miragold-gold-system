...
import { db } from "@/db";
import { users, walletLedger, pendingAllocations, goldPrices, orders, buybackRequests, redemptions } from "@/db/schema";
...
export async function GET() {
  ...
  // Release any abandoned Jual Emas / Tebus Barang Kemas holds first so the
  // On Hold figure below is never stale (Fasa 2A spec section 7; Fasa 2B
  // spec section 12).
  ...
  const [
    ...
    // Fasa 2B spec section 20 — same idea for active redemption quotations
    // (REDEMPTION_ACTIVE_HOLD_STATUSES set in wallet.ts).
    db
      .select({ total: sql<string>`coalesce(sum(${redemptions.gramUsed}), 0)` })
      .from(redemptions)
      .where(
        sql`${redemptions.status} IN ('PENDING_CONFIRMATION','AWAITING_PAYMENT','PAYMENT_CONFIRMED','PROCESSING','READY_FOR_FULFILLMENT')`,
      ),
  ...
  const gramOnHold = new Decimal(buybackOnHoldRow?.total ?? "0").plus(new Decimal(redemptionOnHoldRow?.total ?? "0"));
